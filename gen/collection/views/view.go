...
// RequestToPayResult is the viewed result type that is projected based on a
// view.
type RequestToPayResult struct {
	// Type to project
	Projected *RequestToPayResultView
	// View to render
	View string
}
...
// RequestToPayResultView is a type that runs validations on a projected type.
type RequestToPayResultView struct {
...
var (
	...
	// RequestToPayResultMap is a map of attribute names in result type
	// RequestToPayResult indexed by view name.
	RequestToPayResultMap = map[string][]string{
		"default": []string{
			"amount",
			"currency",
...
// ValidateRequestToPayResult runs the validations defined on the viewed result
// type RequestToPayResult.
func ValidateRequestToPayResult(result *RequestToPayResult) (err error) {
	switch result.View {
	case "default", "":
		err = ValidateRequestToPayResultView(result.Projected)
	default:
		err = goa.InvalidEnumValueError("view", result.View, []interface{}{"default"})
	}
	return
...
// ValidateRequestToPayResultView runs the validations defined on
// RequestToPayResultView using the "default" view.
func ValidateRequestToPayResultView(result *RequestToPayResultView) (err error) {
	if result.Payer != nil {
		if err2 := ValidatePartyView(result.Payer); err2 != nil {
