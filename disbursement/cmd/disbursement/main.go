...
import (
	...
	disbursement "github.com/wondenge/momo-go/disbursement"
...
func main() {
	...
	{
		tokenSvc = disbursement.NewToken(logger)
		validateSvc = disbursement.NewValidate(logger)
		getbalanceSvc = disbursement.NewGetbalance(logger)
		transferSvc = disbursement.NewTransfer(logger)
	}
	...
	)
	{
		...
		transferEndpoints = transfer.NewEndpoints(transferSvc)
	}
	...
	case "local":
		{
			...
			handleHTTPServer(ctx, u, tokenEndpoints, validateEndpoints, getbalanceEndpoints, transferEndpoints, &wg, errc, logger, *dbgF)
		}
